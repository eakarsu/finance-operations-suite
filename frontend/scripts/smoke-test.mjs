...
const adminCookie = await login('admin@finance-ops.local', 'admin123');
const managerCookie = await login('manager@finance-ops.local', 'manager123');
const analystCookie = await login('analyst@finance-ops.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/reporting', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const reporting = await expectJson('/api/entities/reporting', managerCookie);
const rowId = reporting.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/reporting/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/reporting/approve`, {
  method: 'POST',
  headers: {
